...
export default function WishlistPage() {
  ...
  const handleMoveToCart = async (productId, variantId = null, quantity = 1) => {
    try {
      await wishlistService.moveToCart(productId, variantId, quantity);
      await fetchWishlist();
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <Link to="/" className="inline-flex items-center gap-2 bg-teal-600 text-white px-6 py-3 rounded-lg hover:bg-teal-700">
  ...
  <span className="text-sm text-gray-400 line-through">
    ₹{(item.variant?.originalPrice || item.product.originalPrice).toLocaleString()}
  </span>
)}
                  </div>
                  {item.variant && (
                  ...
                  <div className="flex gap-2 mt-4">
                    <button
                      onClick={() => handleMoveToCart(item.product._id, item.variant?._id, 1)}
