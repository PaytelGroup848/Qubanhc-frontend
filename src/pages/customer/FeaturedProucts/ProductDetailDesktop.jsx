    ...
    <div className="group bg-white rounded-2xl border border-gray-100 p-5 hover:border-teal-200 hover:shadow-[0_4px_24px_rgba(13,148,136,0.08)] transition-all duration-300">
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-0.5 bg-gradient-to-r from-teal-50 to-emerald-50 border border-teal-100 px-2.5 py-1 rounded-full">
          ...
          <span className="text-xs font-bold text-teal-600 ml-0.5">{review.rating}.0</span>
        </div>
      </div>
      ...
      {review.helpful !== undefined && (
        <div className="flex items-center gap-1.5 mt-4 pt-3 border-t border-gray-50">
          ...
          <span className="text-xs text-gray-400">{review.helpful} people found this helpful</span>
        </div>
      )}
    </div>
  );
    ...
    <section id="reviews" className="mt-16 scroll-mt-8">
      ...
      <div className="flex items-center gap-3 mb-10">
        ...
        {localReviews.length > 0 && (
          <span className="ml-1 text-sm text-gray-400 font-medium">({localReviews.length})</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <div className="bg-gradient-to-br from-teal-50 via-white to-emerald-50 rounded-3xl p-6 border border-teal-100 sticky top-6">
            {avgRating ? (
              <>
                <p className="text-xs font-bold text-teal-500 uppercase tracking-widest mb-3">Overall Rating</p>
                <div className="flex items-end gap-3 mb-3">
                  <span className="text-7xl font-black text-gray-900 leading-none">{avgRating}</span>
                ...
                <p className="text-sm text-gray-400 mb-6">Based on {localReviews.length} review{localReviews.length !== 1 ? 's' : ''}</p>
                  ...
                  {ratingCounts.map(({ star, count }) => {
                    ...
                    return (
                      <div key={star} className="flex items-center gap-2.5 text-sm">
                        <span className="w-3 text-gray-500 font-semibold text-xs">{star}</span>
                        ...
                        <span className="w-5 text-right text-gray-400 text-xs font-medium">{count}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="text-center py-6">
                ...
                <p className="font-semibold text-gray-700 text-sm">No reviews yet</p>
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          {currentUser ? (
            <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
              ...
              <div className="flex items-center justify-between mt-3">
                <p className="text-xs text-gray-300">{comment.length}/500 characters</p>
                <button
            ...
            <div className="bg-gradient-to-r from-gray-50 to-teal-50/30 border border-dashed border-teal-200 rounded-3xl p-6 flex items-center gap-5">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-bold text-gray-700 text-sm">Login to leave a review</p>
                <p className="text-gray-400 text-xs mt-0.5">Only verified buyers can write reviews.</p>
              </div>
              <Link
                to="/login"
          ...
          {localReviews.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-14 text-center">
              ...
              <p className="text-gray-300 text-xs mt-1">Your review could be the first one!</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <section className="mt-16">
      <div className="flex items-center gap-3 mb-8">
        <div className="h-8 w-1 rounded-full bg-gradient-to-b from-teal-400 to-emerald-500" />
        <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">Specifications</h2>
      </div>
    ...
    <div className="min-h-screen bg-[#f7f9fb]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <nav className="flex items-center flex-wrap gap-1 text-xs sm:text-sm text-gray-400 mb-6 lg:mb-8">
          <Link to="/" className="hover:text-teal-600 transition-colors font-medium">Home</Link>
          ...
          <ChevronRight className="w-3.5 h-3.5 flex-shrink-0" />
          <span className="text-gray-600 font-semibold truncate max-w-[180px] sm:max-w-xs">{product.name}</span>
        </nav>
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex-1 p-5 sm:p-7 lg:p-8 space-y-5">
              ...
              <div className="space-y-1">
                ...
                {discountPercent > 0 && (
                  <p className="text-sm text-emerald-600 font-semibold flex items-center gap-1.5">
                    ...
                    You save {formatPrice(displayOriginalPrice - displayPrice)} on this order
                  </p>
                )}
                    ...
                    {variants.map((variant) => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative p-3.5 rounded-2xl border-2 text-left transition-all duration-200 active:scale-[0.97] ${
                            active
                              ? 'border-teal-500 bg-gradient-to-br from-teal-50 to-emerald-50 shadow-md shadow-teal-100'
                              : 'border-gray-100 bg-gray-50 hover:border-teal-200 hover:bg-teal-50/30'
                          }`}
                        >
                          {active && (
                            <CheckCircle2 className="absolute top-2.5 right-2.5 w-4 h-4 text-teal-500 fill-white" />
                          )}
                          <p className={`text-sm font-bold pr-5 ${active ? 'text-teal-700' : 'text-gray-700'}`}>
                            {variant.name}
                          </p>
                          <p className={`text-sm font-bold mt-0.5 ${active ? 'text-teal-600' : 'text-gray-500'}`}>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-gray-100 bg-gray-50/80 px-4 py-3.5 gap-4">
                <div>
                  <p className="text-[10px] font-extrabold text-gray-300 uppercase tracking-widest mb-2">Qty</p>
                  <QuantitySelector quantity={quantity} setQuantity={setQuantity} max={displayStock} />
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-extrabold text-gray-300 uppercase tracking-widest mb-1">Total</p>
              ...
              <div className="flex items-center gap-3 bg-gradient-to-r from-emerald-50 to-teal-50/60 border border-emerald-100 rounded-2xl px-4 py-3">
                ...
                <p className="text-sm text-emerald-700 font-medium leading-snug">
                  Order now — estimated delivery in <strong>3–5 business days</strong>
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <TrustBadge icon={RotateCcw} label="30-Day Returns" sub="Hassle-free"   />
                <TrustBadge icon={Shield}    label="Secure Payment" sub="256-bit SSL"   />
              </div>
