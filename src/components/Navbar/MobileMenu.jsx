        ...
        <div className="h-full flex flex-col">
              ...
              <div className="mt-5 rounded-xl bg-white/12 border border-white/15 p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold truncate">{user?.name || 'Welcome back'}</p>
                    <p className="text-xs text-teal-50 truncate">{user?.email || role}</p>
                  </div>
                </div>
          ...
          <nav className="flex-1 overflow-y-auto pb-6">
            <div className="p-4 space-y-1">
              <MenuLink to="/" label="Home" onClose={onClose} icon={Home} active />
              <MenuLink to="/contact" label="Contact" onClose={onClose} icon={Phone} />
              <MenuLink to="/cart" label="Cart" onClose={onClose} icon={ShoppingCart} count={cartCount} />
              <MenuLink to="/wishlist" label="Wishlist" onClose={onClose} icon={Heart} count={wishlistCount} />
              {showDashboard && (
                <MenuLink
                  to={dashboardLink}
...
function MenuLink({ to, label, onClose, icon: Icon, count = 0, active = false, highlight = false }) {
  return (
    <Link
      to={to}
      onClick={onClose}
    ...
    >
      <div className="relative aspect-square rounded-xl overflow-hidden bg-gray-100 shadow-sm ring-1 ring-black/5 group-active:scale-[0.98] transition">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-2">
          <p className="text-white text-xs font-bold line-clamp-2">{category.title}</p>
        </div>
      </div>
